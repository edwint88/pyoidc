...
class Provider(object):
    ...
    def __init__(self, name, sdb, cdb, authn_broker, authz, client_authn,
        ...
        if capabilities:
            ...
            self.capabilities = self.message_factory.get_response_type('configuration_endpoint')(**capabilities)
        else:
    ...
    def verify_capabilities(self, capabilities) -> bool:
        ...
        if not_supported:
            logger.error("Server does not support the following features: %s", not_supported)
            return False
        return True
    ...
    def provider_features(self, pcr_class=None, provider_config=None):
        ...
        if pcr_class is not None:
            warnings.warn('`pcr_class` is deprecated, please use `message_factory`.', DeprecationWarning, stacklevel=2)
        else:
